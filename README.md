...
The September 24 accessibility maintenance pass repairs all named Evidence section destinations, supplies native section links and visible targets for older shared URLs, derives exchange metadata from the records, and makes page titles distinguishable at a glance. Chronology and alternative headings now reflect their grouping. Meeting headings include dates, with fallback-link explanations beside their links and connected by accessible descriptions. Numeric timeline IDs are explicit and retain their published values. The archive pages retain their lean navigation and original preparation dates, with consistent About and contact links. Entry-link and copy-link controls remain removed. This pass does not change the paper, source records, source-check dates, or research baseline.

The September 24 update adds the 2024 survey’s reported 57% no and 43% yes responses on whether a vertical barrier was needed, separately from the effectiveness question. The survey charts on PDF p. 19 were visually checked, and the existing participation and question-response limits remain beside the results. The Who said what metadata now matches the existing 75 entries. The site-update date advances to September 24 without changing the comprehensive research baseline or unrelated source-check dates.
...
The checks cover both readable sources and the production bundle, including data, routing, page locators, source-note search, static pages, menu interactions, and enlarged-header offsets. They verify 75 entries (including the legacy 35-entry group), all 23 speakers, year filtering, removal of optional entry-sharing controls, Overview-only introduction, distinct excerpt verification labels, and dated follow-ups retained while record-note blocks and the recurring-question callout remain excluded from views and search. The page checks cover direct loads, local links at both deployment roots, old entry links, new-tab behavior, back/forward, shared rendering, the email and 988 links, metadata, and the four illustrations. They are not a substitute for browser or physical-device testing.

At the author's subsequent request, the repeated “Entry link” and “Copy entry link” controls were removed from all 71 selected exchanges, together with their unused clipboard handler and status message. Existing entry IDs and routes remain available to search, date jumps, meeting links, and previously shared URLs. Recordings, supporting documents, source notes, quotations, and the newer topic/meeting reading paths are unchanged. This is an interface simplification, not a source removal.

The accepted independent-review follow-up fixes “Skip to content” so it focuses the existing reading area without changing the route, filters, search terms, or open disclosures. This also covers legacy hash links. The source-table return link now uses the Meetings & documents path. A compact “Who decides what?” disclosure on Timeline, linked from Overview and search, distinguishes staff/design work, advisory commissions, Public Safety Committee review, Council concept approval, and subsequent design/construction requirements. Its sources are the November 2023 presentation (PDF p. 34), August 2021 report (PDF p. 4), August 2026 project row (PDF p. 184), and City project page. The explanation does not establish a scheduled next decision or claim a fresh overall status review. It adds one searchable record, bringing the total to 140, without restoring excluded news entries.
