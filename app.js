...
const timeline = [
  ...
  {id:'5',date:'2022–2023',title:'The design process restarted with another consultant',text:'The September 2022 report scheduled a Public Safety Committee return for September 2023. The first request for proposals received no proposals. A February 2023 newsletter forecast a return by year-end. The City invited proposals again and awarded the Apexx Architecture contract on April 14, 2023.',result:'The committee-return forecast had moved from September to the end of 2023 before the new concepts were presented.',page:12,refs:'4, 13, 24',links:[['Public Safety · September 21, 2022 · Schedule · PDF p. 3',urls.r2022+'#page=3'],['City Manager · February 9, 2023 · Newsletter · PDF p. 2',urls.rfp]]},
  {id:'6',date:'Nov 2023',title:'Three new concepts, more questions, and further review',text:'Canted webmesh, vertical webmesh, and metal pickets were presented on November 15. Members again asked about netting, landscaping, staffing, and whether deaths would move elsewhere.',result:'The return came after the original September 2023 target but within the later year-end forecast. Commission reviews and a survey continued. No permanent design was selected.',page:13,refs:'13–15',links:[['Public Safety · November 15, 2023 · Concepts',urls.p2023],['February 2023 forecast · PDF p. 2',urls.rfp],['Meeting recording',urls.v2023]]},
  {id:'7',date:'Jan–Jul 2024',title:'Review narrowed the options without a final selection',text:'The Design Commission reached no consensus in January. By July, staff’s options had narrowed to vertical webmesh and metal pickets, and staff returned with patrol costs, netting analysis, rescue concerns, and survey results.',result:'No permanent design was sent to the full Council. Further design work, feedback, and reviews were scheduled.',page:15,refs:'16–22, 33',links:[['January minutes',urls.mjan24],['July minutes',urls.m2024]]},
  {id:'8',date:'2025–2026',title:'The design schedule moved again',text:'City project reports moved the target for finishing the design to June 30, 2028. The report covering work through June 30, 2026 said the options were still being evaluated and design work was continuing.',result:'June 2028 was a design-phase target. It was not a funded construction-completion commitment.',page:19,refs:'26',links:[['Year-end project row · p. 184',urls.q426]]},
  {id:'9',date:'Reviewed record',title:'Design and construction remained separate decisions',text:'The reviewed records showed no final permanent-design approval or construction award. The City project page said design was funded and construction funding remained to be identified.',result:'This describes the reviewed records. It is not a new live-status check.',page:20,refs:'26–28, 31, 34',links:[['City project page',urls.project],['April 2026 funding discussion',urls.v2026]]}
];

// Explicit IDs keep every published numeric timeline URL stable after future insertions.
...
function pageMetadata({view,arg}) {
  ...
  return {title:label+' | Colorado Street Bridge Project Guide',description,canonical:'https://coloradostreetbridgeproject.com/'+(sectionPaths[view]?sectionPaths[view]+'/':'')+(isTopic?arg+'/':''),robots:view==='search'?'noindex, follow':'index, follow'};
}
...
function directoryLinks(items,recordId) {
  return '<ul class="directory-links">'+items.map((item,index)=>{
...
function viewMarkup({view,arg,filter='all',year='all',order='oldest',query=''}) {
  ...
  // Older shared section URLs retain a visible native target without JavaScript.
  if(view==='evidence')markup=markup.replace(/(<section\b[^>]*\bid="([^"]+)"[^>]*>[\s\S]*?<h2[^>]*>)([^<]+)(<\/h2>)/g,(whole,start,id,title,end)=>evidenceSections.includes(id)?`${start}<span id="evidence/${esc(id)}" class="route-anchor">${title}</span>${end}`:whole);
  return markup;
}
function render(focus=false) {
  ...
  if(view==='evidence' && evidenceSections.includes(arg))target=document.getElementById(arg);
