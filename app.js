...
const topics = {
  ...
  technology: {
    ...
    limit:'A question, presentation, or request for study does not establish that a system was funded, installed, staffed, or evaluated. This guide makes no claim about a currently operating remote-intervention system.',limitPage:19,limitRefs:'16, 18, 20–21'
  }
};
...
const timeline = [
  ...
  {date:'May 2019',title:'The first design contract was approved',text:'The Council approved the Donald MacDonald Architects contract after a revised proposal shortened the design schedule by ten months and reduced the proposed contract from $700,000 to $500,000. The forecast placed Council concept approval in December 2019 and construction in August 2020, pending budget appropriation. These were forecasts, not funded construction commitments.',result:'The anticipated schedule put Council concept approval in December 2019 and construction in August 2020, pending budget appropriation. These were forecasts, not completed milestones or funded construction commitments.',page:8,refs:'8',links:[['May 2019 staff report · PDF p. 4',urls.r2019+'#page=4']]},
  ...
  {date:'2022–2023',title:'The design process restarted with another consultant',text:'The September 2022 report scheduled a Public Safety Committee return for September 2023. The first request for proposals received no proposals. A February 2023 newsletter forecast a return by year-end. The City reissued the solicitation and awarded the Apexx Architecture contract on April 14, 2023.',result:'The committee-return forecast had moved from September to the end of 2023 before the new concepts were presented.',page:12,refs:'4, 13, 24',links:[['Public Safety · September 21, 2022 · Schedule · PDF p. 3',urls.r2022+'#page=3'],['City Manager · February 9, 2023 · Newsletter · PDF p. 2',urls.rfp]]},
...
function overview(){return head('01','Why is the fence still there?','In April 2018, the City Council chose to pursue a permanent barrier. The temporary fence installed in September 2018 remains on the bridge.')+`
  <article class="feature-card lead overview-summary"><p class="eyebrow">THE SHORT VERSION</p><p class="large">As of September 2026, the records reviewed for this guide do not show an approved permanent design.</p><p>The temporary fence stayed while designs went through mockups, commission reviews, public input, a change of consultant, and further study. Construction also required funding separate from the design budget.</p>
  <dl class="project-status"><div><dt>Permanent design</dt><dd>No final approval established in the reviewed records.<small>City project page checked ${formatDate(reviewDates.projectPage)}.</small></dd></div><div><dt>Design-phase target</dt><dd>June 30, 2028.<small>Report covering activity through June 30, 2026. Not a construction-completion date.</small></dd></div><div><dt>Construction funding</dt><dd>Still to be identified in the reviewed City project page.<small>Checked ${formatDate(reviewDates.projectPage)}. A request is not an award.</small></dd></div></dl></article>
  <nav class="next-cards" aria-label="Explore the project"><a class="next-card" href="${esc(routeHref('timeline'))}" data-go="timeline"><strong>How did the schedule move?</strong><span>Compare the May 2019 forecast of construction in August 2020, pending appropriation, with what followed.</span><span class="action">The timeline →</span></a><a class="next-card" href="${esc(routeHref('alternatives'))}" data-go="alternatives"><strong>What alternatives were studied?</strong><span>See the vertical designs, then follow the reviews of netting, trees, patrols, and cameras.</span><span class="action">The alternatives →</span></a><a class="next-card" href="${esc(routeHref('evidence'))}" data-go="evidence"><strong>Why would a barrier help?</strong><span>Read the prevention research and the questions it does not settle.</span><span class="action">The evidence →</span></a></nav>
...
function designGallery(){return `<section class="design-gallery" aria-labelledby="design-gallery-title"><h2 id="design-gallery-title">The fence and the vertical designs</h2><p class="topic-orientation">The temporary chain-link fence is different from the permanent concepts. These images come from the City’s July 17, 2024 presentation. They are shown together to explain their appearance, not as a current shortlist.</p><div class="design-grid">
${[
['temporary-fence','Temporary chain-link fence',6,1389,525,'Photograph looking along the bridge roadway, with chain-link fencing on both sides.','Photograph reproduced on slide 6. Its capture date is not stated. This is not a photograph taken for the September 2026 update.'],
['metal-pickets','Metal picket concept',12,734,820,'City concept rendering with closely spaced vertical metal pickets beside the bridge lamps.','Concept rendering, slide 12. Not an installed permanent barrier.'],
['vertical-webmesh','Vertical webmesh concept',13,734,820,'City concept rendering with upright posts supporting fine mesh along the balustrade.','Concept rendering, slide 13. Not an installed permanent barrier.'],
['canted-webmesh','Canted webmesh concept',14,734,820,'City concept rendering with angled posts supporting mesh over the edge of the sidewalk.','Concept rendering, slide 14. Shown for comparison. The commission-feedback summary on slide 16 identifies this option as eliminated.']
...
</div><p class="locator-note">Earlier curved-mesh mockups and the February 2020 enclosure request belong to different stages of the project. <a href="${esc(routeHref('timeline/2020-02-03'))}" data-route="timeline/2020-02-03">Read the February 2020 record.</a> Reproduced for explanation, with original attribution retained.</p></section>`;}

function alternatives(key){const t=topics[key]||topics.netting;return head('03','What alternatives were studied?','Start with the vertical designs, then select another approach to follow its study and the responses.')+`
  ${key?'':designGallery()}<h2>Other approaches studied</h2><p class="topic-orientation">${key?`<a href="${esc(routeHref('alternatives'))}" data-route="alternatives">See the fence and vertical design concepts</a>`:'The Council’s permanent-barrier direction provides the context for these reviews of other approaches.'}</p><div class="topic-layout"><nav class="topic-menu" aria-label="Approaches considered">${Object.entries(topics).map(([id,x])=>`<a href="${esc(routeHref('alternatives/'+id))}" data-topic="${id}" aria-current="${t===x?'page':'false'}">${esc(x.name)}<small>${esc(x.sub)}</small></a>`).join('')}</nav><div class="topic-main"><article class="topic-answer"><span class="pill">${esc(t.tag)}</span><h3 id="topic-title" class="scroll-focus" tabindex="-1">${esc(t.title.replace(/\.$/,''))}</h3><p>${esc(t.answer)}</p></article>${steps(t.steps)}<div class="note"><p><strong>Limits of this finding</strong></p><p>${esc(t.limit)}</p>${citations(t.limitPage,t.limitRefs)}</div></div></div>`;}
...
function remarkExcerpt(remark) {
  ...
  let verification=verified?'Previously checked against the recording.':'Quoted excerpt; the source note states what was checked.';
  ...
  if(['madison-response','gordo-staffing'].includes(remark.id))verification='The author checked speaker handoffs and locators against the recording, not every quoted word.';
...
function remarkCard(key, person, remark) {
  ...
  return `<article class="remark-card" id="${esc(remark.id)}" tabindex="-1">
    <div class="remark-top"><p class="remark-person">${esc(person.name)}<span class="speaker-role">${esc(speakerRole(person))}</span></p><time class="remark-date" datetime="${esc(remark.sortDate)}">${esc(remark.date)}</time><span class="remark-topic">${esc(speakerTopics[remark.topic])}</span></div>
...
function speakerView(key='all', topic='all', year='all') {
  ...
  return head('05','Questions, answers, and decisions','Follow the selected exchanges in date order. Read what had already been studied, what was asked next, and how staff or other speakers responded.')+`
    ...
    <p class="locator-note">Roles describe the period of these selections, not necessarily current positions.</p>
...
function searchIndex() {
  ...
  for (const [view,html] of [['evidence',evidence()],['overview',overview()]]) {
    ...
    div.querySelectorAll('article.feature-card,aside.feature-card').forEach((a,i)=>{
      ...
      result.push({type:view==='evidence'?'Evidence & limits':'Overview',title:h?h.textContent:'Project overview',text:a.textContent,route:view+'/'+(a.dataset?.evidenceId??i)});
    });
  }
...
function aboutView() {
  return `<div class="section-head"><div><h2>About this guide</h2></div></div>
    <div class="info-copy">
      ...
      <h2>The guide and the paper</h2><p>This website is a navigable guide to the research behind <cite>The fence everyone can see</cite>. The full paper is not published here. The links beside each entry identify the records supporting that entry.</p><h2>Reading the source labels</h2><p>Quotations, City-caption excerpts, working-transcript excerpts, and summaries are labeled separately. Each source note describes the check performed. Confirming a speaker or a passage time does not, by itself, verify every quoted word.</p>
...
function forecastComparison(){return `<section aria-labelledby="forecast-heading"><h2 id="forecast-heading">Forecasts and the later record</h2><div class="forecast-table" role="region" aria-label="Schedule comparison" tabindex="0"><table><caption>Forecast dates were not guarantees. The design phase and construction are separate milestones.</caption><thead><tr><th scope="col">Documented forecast</th><th scope="col">What the later record shows</th></tr></thead><tbody>
<tr><th scope="row">May 2019: concept approval in December 2019; construction in August 2020, pending appropriation.</th><td>Mockups reached the committee in August 2021, but no permanent design was selected. ${link('May 2019 report · p. 4',urls.r2019+'#page=4')} ${link('August 2021 report',urls.r2021)}</td></tr>
<tr><th scope="row">September 2022: committee return in September 2023. February 2023: return by year-end.</th><td>New concepts were presented on November 15, 2023: after the earlier target, within the revised forecast. ${link('September 2022 report · p. 3',urls.r2022+'#page=3')} ${link('February 2023 newsletter · p. 2',urls.rfp+'#page=2')} ${link('November 2023 presentation',urls.p2023)}</td></tr>
<tr><th scope="row">Later capital-project reports: design-phase target of June 30, 2028.</th><td>The report covering activity through June 30, 2026 still described concept evaluation and an ongoing design phase. This is not a construction-completion commitment. ${link('Year-end project row · p. 184',urls.q426)}</td></tr>
...
function viewMarkup({view,arg,filter='all',year='all',order='oldest',query=''}) {
  ...
  let markup=view==='overview'?overview():view==='timeline'?head('02','Agreement did not finish the project','Follow the sequence from the emergency response to the later design and funding questions.')+forecastComparison()+'<p class="timeline-key">Expand an entry for its supporting record.</p>'+steps(timeline,true):view==='alternatives'?alternatives(Object.hasOwn(topics,arg)?arg:undefined):view==='evidence'?evidence():view==='speakers'?speakerView(person,filter,year):view==='meetings'?meetingsView(year,order):view==='news'?newsView():view==='about'?aboutView():view==='changes'?changesView():searchView(query);
