...
function directoryLinks(items,recordId) {
  return '<ul class="directory-links">'+items.map(item=>{
    ...
    return `<li><a class="source-link" href="${esc(url)}" target="_blank" rel="noopener noreferrer"><span class="source-label">${esc(label)}${linkArrow(url)}</span>${item.note?'<small>'+esc(item.note)+'</small>':''}</a>${files?'<p class="file-locator">In that folder, select '+files.map(file=>'<code>'+esc(file)+'</code>').join(' or ')+'.</p>':''}</li>`;
  }).join('')+'</ul>';
}
