...
function remarkExcerpt(remark) {
  ...
  if(/caption/i.test(remark.kind))verification=remark.id==='jones-continue'?'Words from the City’s video captions. The author checked who was speaking and where the passage appears in the recording.':'Words from the City’s video captions. See the source note for the documented recording checks.';
  if(/transcript/i.test(remark.kind))verification='Words from the working transcript. See the source note for the documented recording checks.';
...
function speakerView(key='all', topic='all', year='all') {
  ...
  return head('05','Questions, answers, and decisions','Read selected exchanges in date order, with their background, responses, and supporting records.')+`
    ...
    ${groups.length>1?`<details class="meeting-jumps"><summary>Jump to a meeting (${groups.length})</summary><nav aria-label="Selected meetings">${groups.map(g=>{const entry=g.items[0];const params=new URLSearchParams();if(topic!=='all')params.set('topic',topic);if(year!=='all')params.set('year',year);const route='speakers/'+key+'/'+entry.remark.id+(params.toString()?'?'+params:'');return `<a href="${esc(routeHref(route))}" data-route="${esc(route)}">${esc(g.date)} · ${esc(g.body)} (${g.items.length})</a>`;}).join('')}</nav></details>`:''}
...
function aboutView() {
  return `<div class="section-head"><div><h2>About this guide</h2></div></div>
    <div class="info-copy">
      ...
      <h2>The guide and the paper</h2><p>This website helps readers explore the research behind <cite>The fence everyone can see</cite>. The full paper is not published here.</p><h2>Reading the source labels</h2><p>Quotations, City-caption excerpts, working-transcript excerpts, and summaries are labeled separately. Source notes preserve the author’s recording checks and identify the scope documented for each selection. Checking who was speaking and when does not mean every quoted word was checked against the audio.</p>
